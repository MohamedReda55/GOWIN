...
class Parser(object):
    ...
    def list_expression(self):
        """
        list_token                  : LBRACKET elements RBRACKET
        elements                    : element (COMMA element)*
        element                     : primary_expression
        """
        elements = []
        self.eat(LIST_START)
        while self.current_token.type != RBRACKET:
            elements.append(self.primary_expression())
    ...
    def parse_list(self,list_token):
        values_li = []
  
        for child_token in list_token.values:
            if child_token.type == ID:
                values_li.append(Var(
                    token=self.current_token,
                    line=self.lexer.line
                ))
            elif child_token.type in [INTEGER_CONST, CHAR_CONST, FLOAT]:
                values_li.append(Num(
                    token=child_token,
                    line=self.lexer.line
                ))
            elif child_token.type == STRING:
                values_li.append(String(
                    token=child_token,
                    line=self.lexer.line
                ))
            elif child_token.type == LIST:
                values_li.append(self.list_token(child_token))
    ...
    def list_token(self):
        ...
        """
        token = self.current_token
        values_li=self.parse_list(token)
        token.values=values_li
        self.eat(LIST)
        return List(
            token=token,
            line=self.lexer.line
    ...
    def parse(self):
        ...
        if self.current_token.type != EOF:
            self.error("Expected token <EOF> but found <{}>".format(
